...
**Department:** BTech Computer Science & Engineering, Sanjivani College of Engineering  
...
| Class | Raw Image Count | Final Dataset Split Strategy |
| :--- | :--- | :--- |
| **AMD** | 112 | Stratified 70/15/15 Split |
| **Cataract** | 333 | Pruned via SSIM → Stratified Split |
| **Dementia** | 397 | Pruned via SSIM → Stratified Split |
| **Diabetic Retinopathy** | 63 | Data Augmentation → Stratified Split |
| **Glaucoma** | 105 | Stratified 70/15/15 Split |
| **Normal** | 76 | Stratified 70/15/15 Split |
| **Total** | **1,086 Raw** | **974 Balanced Model-Ready Images** |
...
| **EfficientNetV2-S** | Convolutional | **94.63%** | Lightweight |
| **ResNeXt50** | Convolutional | **87.92%** | Standard |
| **FNet** | Fourier-mixing | Exported `.pth` | Ultra-fast Fourier |
| **Perceiver IO** | Attention-based, latent bottleneck | Exported `.pth` | Latent Bottleneck |
...
- **Processor**: Raspberry Pi 5 (Core Processor, Offline Inference, Lightweight OS).
- **Display**: 7-inch LCD Monitor (Responsive GUI, Dropdown Model Select, Live Feed Overlay).
...
The project repository is organized to separate data, application code, evaluation tooling, and documentation into clearly delineated folders:
- `Dataset`: Holds the raw and processed fundus images along with dataset-level documentation and Kaggle dataset reports.
- `App`: Contains the packaged executable, the PyQt6 source code (`main.py`, `models/`, `ui/`, `utils/`), installer scripts (`ISS/`), and additional resources including model weight pointers.
- `Output`: Stores generated inference images, output screenshots, and diagnostic results.
- `Evaluation`: Contains the inference script (`inference.py`) used to run and assess the models.
- `Documentation`: Holds this comprehensive document and the accompanying presentation slides.
